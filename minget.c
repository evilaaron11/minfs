...
char *imageName, *pathName, *outFile;

void usageMessage() {
   printf("usage: minget [-v] [-p num [-s num] ] ");
   printf("imagefile minixparh [hostpath]\n");
   printf("Options: \n");
...
int fileRead(int zoneNum, uint16_t zonesize, uint16_t size,
      FILE *image, struct dir **files, uint32_t lFirst) {
   int i, offset = zonesize * zoneNum + lFirst * SECTOR_SIZE;
   int numFiles = size / DIR_SIZE, currSize = 0;
   char *file;
   ...
   /* Meeds to expand to multiple zone support */
   fseek(image, offset, SEEK_SET);
   for (i = 0; i < size && currSize < zonesize; i++) {
      fread(file, DIR_SIZE, 1, image);
      currSize += DIR_SIZE;
      printf("%s",file);
      file++;
   }
...
struct inode getiNode(FILE *image, int blocksize, 
	uint32_t lFirst, int inodeNum) {
   struct inode root;
   int offset = (2 * blocksize) + iNodeMapSize + zoneMapSize
      + (lFirst * SECTOR_SIZE);
...
void getFile(struct inode in, uint16_t zonesize,
      FILE *image, struct dir **files, uint32_t lFirst) {
   int i, sizeLeft = in.size, currFile = 0;
   struct dir *temp;
   for (i = 0; sizeLeft != 0 && i < DIRECT_ZONES; i++) {
      temp = *files + currFile;
      sizeLeft = fileRead(in.zone[i], zonesize, sizeLeft, image,
            &temp, lFirst);    
      currFile = (in.size - sizeLeft) / DIR_SIZE;
...
struct inode parsePath(char *path, uint16_t zonesize, FILE *image,
   ...
   while (curr != NULL) {
      ...
      getFile(root, zonesize, image, &temp, lFirst);
...
int main (int argc, char **argv) {
   ...
   if (part >= 0) {
      ...
      if (subpart >= 0) {
         ...
         // printf("First Sector of subpartition is %d\n", firstSector);
      }
   }
